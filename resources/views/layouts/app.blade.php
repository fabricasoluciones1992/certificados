...
<head>
    ...
    <link href="{{ asset('css/style.css') }}" rel="stylesheet">    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.4/jquery.min.js"></script>
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <ul class="navbar-nav ms-auto">
                        ...
                        @if(Auth::user()->id_roles =='2')
                            <li class="nav-item">
                                <a class="nav-link active text-light rounded ms-3 {{request()->routeIs('contracts.index') ? 'active' : ''}}" aria-current="page" href="{{route('contracts.index')}}">Contratos</a>
                            </li>

                            <li class="nav-item">
                                <a class="nav-link active text-light rounded ms-3 {{request()->routeIs('areas.index') ? 'active' : ''}}" aria-current="page"  href="{{route('areas.index')}}">Áreas y cargos</a>
                            </li>

                            <li class="nav-item">
                                <a class="nav-link active text-light rounded ms-3 {{request()->routeIs('customers.index') ? 'active' : ''}}" aria-current="page" href="{{route('histories')}}">Historial Certificados</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link active text-light rounded ms-3 {{request()->routeIs('customers.index') ? 'active' : ''}}" aria-current="page" href="{{route('users.index')}}">Control usuarios</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link active text-light rounded ms-3 {{request()->routeIs('select_contract') ? 'active' : ''}}" aria-current="page" href="{{route('select_contract')}}">Generar certificado</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link active text-light rounded mx-2 px-3 {{request()->routeIs('customers.index') ? 'active' : ''}}" aria-current="page" href="{{route('home')}}">Inicio</a>
                            </li>
                            @endif
                        @endauth
                        @guest
                        ...
                        @else
                            <li class="nav-item dropdown me-2">
                                <a id="navbarDropdown" class="nav-link dropdown-toggle text-light rounded px-2" href="#" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                                    @if( Auth::user()->name == '0' )
                                        {{-- Mientras el usuario no tenga un rol dentro de la empresa no vera informaciòn suya dentro del sistema {{ Auth::user()->email }} --}}
                                    @else
                                        {{ Auth::user()->name }}
                                    @endif
                                </a>
