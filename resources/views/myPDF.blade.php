...
<body>
  ...
  <div class="test">
    <h3 >{{ $title }}</h3>
            ...
            <p class="textContent ">Que el Señor <b class="container">{{ $name }}</b>, identificado con <b> {{ $t_doc }} </b> No. <b>{{ $document }}</b>; está vinculado con la Fundación Universitaria Empresarial de la Cámara de Comercio de Bogotá Uniempresarial con NIT 830.084.876-6; desempeñando el cargo de <b>{{ $contract->posts->name }}</b>.
            {{$type_contract}}
            {{$date}}
            {{$salary}}
            </p>
